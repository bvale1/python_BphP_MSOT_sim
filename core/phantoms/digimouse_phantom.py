...
class digimouse_phantom(phantom):
    ...
    def calculate_tissue_absorption_coefficients(self, coupling_medium_mu_a) -> np.ndarray:
        return np.array([
            coupling_medium_mu_a * np.ones(len(self.wavelengths_m)), # 0 --> background
    ...
    def calculate_tissue_scattering_coefficients(self, coupling_medium_mu_s) -> np.ndarray:
        return np.array([
            coupling_medium_mu_s * np.ones(len(self.wavelengths_m)), # 0 --> background
            ...
            coupling_medium_mu_s * np.ones(len(self.wavelengths_m)), # 13 --> bladder --> water, Hendrik Buiteveld (1994)
    ...
    def create_volume(self,
        ...
        #mu_s_jac = lambda a, b : (a * ((wavelengths_nm/500)**(-b))) * 1e3 # Steven L Jacques (2013) 
        ...
        absorption_coefficients = self.calculate_tissue_absorption_coefficients(coupling_medium_mu_a)
        scattering_coefficients = self.calculate_tissue_scattering_coefficients(coupling_medium_mu_s)
    ...
    def get_tissue_types_dict(self) -> dict:
        ...
        for i, tissue_type in enumerate(self.calculate_tissue_absorption_coefficients(0.1)):
            tissue_types_dict[keys[i]]['mu_a'] = tissue_type
        for i, tissue_type in enumerate(self.calculate_tissue_scattering_coefficients(100)):
