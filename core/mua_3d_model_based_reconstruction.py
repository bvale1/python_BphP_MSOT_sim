...
if __name__ == '__main__':
    ...
    #    default='/mnt/fast/nobackup/users/wv00017/ImageNet_fluence_correction/digimouse_3d_mua_model_based_reconstruction_checkpoint_file.json',
    ...
    parser.add_argument(
        '--wavelength_nm', type=float, default=None, action='store',
        help='wavelength of light source in nm'
    )
    parser.add_argument(
        '--y_scanning_step', default=25, action='store',
        help='step size along y axis for scanning the digimouse atlas in voxels'
    )
    parser.add_argument(
        '--y_scanning_start', default=200, action='store',
        help='start position along y axis for scanning the digimouse atlas in voxels'
    )
    parser.add_argument(
        '--y_scanning_end', default=775, action='store',
        help='end position along y axis for scanning the digimouse atlas in voxels'
    )
    ...
    if (os.path.exists(args.save_dir+'config.json') 
        ...
        # this computes the grid size and dx for simulating a single scanning postion
        ...
        # compute grid size to include all scanning positions
        y_positions = np.arange(
            args.y_scanning_start, 
            args.y_scanning_end + args.y_scanning_step,
            args.y_scanning_step
        )
        mcx_grid_size[1] = mcx_grid_size[1] + abs(args.y_scanning_end - args.y_scanning_start)
        ...
        cfg = {
            ...
            'nphotons' : 1e8,
            'nsources' : 10,
            'wavelengths' : [args.wavelength_nm * 1e-9], # [m]
            ...
            'y_scanning_step' : args.y_scanning_step, # step size along y axis for scanning the digimouse atlas in voxels
            'y_scanning_start' : args.y_scanning_start, # start position along y axis for scanning the digimouse atlas in voxels
            'y_scanning_end' : args.y_scanning_end, # end position along y axis for scanning the digimouse atlas in voxels
        }
        
        logging.info(f'no checkpoint, creating config {cfg}')
        
        # initialise random number generator, for digimouse the seed is used
        # to sample laser energy and if stochastic noise is added to the sensor data
        if cfg['seed']:
    ...
    with h5py.File(cfg['save_dir']+'data.h5', 'r+') as f:
        ...
        if 'mu_a' not in f[h5_group]:
            # pressure is cropped to crop_p0_3d_size but mu_a, mu_s and bg_mask are cropped to crop_size
            f[h5_group].create_dataset(
                'mu_a',
                data=uf.crop_p0_3D(
                    volume[0,:,:,:], [cfg['crop_size'], cfg['kwave_grid_size'][1], cfg['crop_size']]
                ), dtype=np.float32
            )
        if 'mu_s' not in f[h5_group]:
            f[h5_group].create_dataset(
                'mu_s',
                data=uf.crop_p0_3D(
                    volume[1,:,:,:], [cfg['crop_size'], cfg['kwave_grid_size'][1], cfg['crop_size']]
                ), dtype=np.float32
            )
        if 'bg_mask' not in f[h5_group]:
            f[h5_group].create_dataset(
                'bg_mask', data=uf.crop_p0_3D(
                    bg_mask, [cfg['crop_size'], cfg['kwave_grid_size'][1], cfg['crop_size']]
                ), dtype=bool
            )
